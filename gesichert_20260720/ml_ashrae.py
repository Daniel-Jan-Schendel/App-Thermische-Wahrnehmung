...
with tab2:
    ...
    st.markdown("&nbsp;&nbsp;&nbsp;&nbsp; - Einsatz von TomekLink Filtern und SMOTE (Schärfen der Klassengrenzen und Data Augmentation)")
...
with tab3:
    ...
    # 2. Vorhersagen für die Metriken generieren
        ...
        shap_sampleTH = transformed_df.sample(min(100, len(transformed_df)), random_state=42)
        explainerTH = shap.TreeExplainer(rf_model)
        shap_valuesTH = explainerTH.shap_values(shap_sampleTH)
        ...
        experiment_data = {
            ...
            'X_train': X_train,              
            'transformed_X_train': transformed_df,
            'y_test': y_test,
            'y_pred': y_pred,
            'y_train': y_train,
    ...
    #        st.session_state['best_params'] = best_params
        ...
        with col2:
            ...
            with st.spinner("Generiere SHAP-Analyse... Bitte warten..."):
                ...
                shap_sampleTH = transformed_df.sample(min(400, len(transformed_df)), random_state=42) # nur 400 samples!
                explainerTH = shap.TreeExplainer(rf_model)
                shap_valuesTH = explainerTH.shap_values(shap_sampleTH)
